...
import { useRegisterResponsibleArea } from 'data/use-case';
...
interface RegisterResponsibleAreaFormProps {
  closeModal: () => void;
  responsibleArea?: ResponsibleArea;
}

export const RegisterResponsibleAreaForm: FC<RegisterResponsibleAreaFormProps> = ({
  closeModal,
  responsibleArea
}) => {
  const {
  ...
  useEffect(() => {
    ...
    if (responsibleArea) {
      const { name, description, status } = responsibleArea as ResponsibleArea;
      setValue('name', name);
      setValue('description', description);
      setValue('status', status);
    }
  }, [setValue, responsibleArea, companyId]);
    ...
    <form className={'flex flex-col gap-5 w-full'} onSubmit={handleSubmit(onSubmit)}>
      <div className={'flex flex-col gap-3 w-full'}>
        ...
        <div className={'flex w-full items-center justify-between'}>
          <p className={'w-full'}>Situação atual da área responsável</p>
          <SwitchController
            control={control}
            name={'status'}
            options={[
